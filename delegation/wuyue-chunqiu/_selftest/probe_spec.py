...
CASES: list[tuple[str, str, str, str]] = [
    ...
    ("S1 說辭側與過度側表標題被改名",
     ...
     "### 說辭與過度（4 段，非空但一格都不得填 XII）", "S1"),

    ("S2 說辭側與過度側標題宣告段數不符",
     "### 說辭側與過度側（4 段，非空但一格都不得填 XII）",
     "### 說辭側與過度側（5 段，非空但一格都不得填 XII）", "S2"),
    ...
    ("S3 說辭側錨點段號指到別段",
     "| `勾踐十三年`[3] | 「天若棄吳，必許王矣。」 |",
     "| `勾踐十三年`[4] | 「天若棄吳，必許王矣。」 |", "S3"),

    ("S3 過度側錨點引句對不上原文",
     "「有敢侵之者，上天所殃」", "「有敢侵之者，上天所罰」", "S3"),
    ...
    ("S8 A 類 6b 條文段數與表脫鉤",
     "**說辭側與過度側 4 段 `domains` 全部非空",
     "**說辭側與過度側 3 段 `domains` 全部非空", "S8"),

    ("S8 A 類第 7 條的 X 命中側段數與表脫鉤",
     "**X 命中側 9 段全部含 X",
     "**X 命中側 4 段全部含 X", "S8"),

    ("S14 說辭側與過度側沒有 A 類條文引用",
     "**說辭側與過度側 4 段 `domains` 全部非空",
     "**這四段 `domains` 全部非空", "S14"),
    ...
    ("S16 來源殘留全表指到錯的段",
     "`闔閭十年`[14] `(魚且)`", "`闔閭十年`[13] `(魚且)`", "S16"),

    ("S16 來源殘留內容與原文對不上",
     "`第006卷`[26] `(手改目)`", "`第006卷`[26] `(目改手)`", "S16"),

    ("S16 來源殘留全表宣告處數錯",
     "**來源殘留全表（4 處）**：", "**來源殘留全表（5 處）**：", "S16"),
