...
`translations/` 已有 **76 部**（phase 1 共 68 ＋ phase 2 小學 4 ＋ 2026-08-24 自 religions-history 收入諸子 5：`huainanzi`／`mozi`／`wenzi`／`jiayi-xinshu`／`lujia-xinyu`；`guangyun` 上游殘缺已在 catalog 標 `excluded`），`00-overview/` 已生成。`annotations.json` 目前 **52 部、23,367 段**（判讀 23,257，`cai-zhonglang-ji`〈外集卷四〉110 段留 `null`）。**逐書段數不在本檔列舉——那份清單會鏽掉，一律讀生成物 `00-overview/INDEX.json` 的 `texts[]`。**

已標完的成組里程碑：phase 2 小學四部、兵家五部（299 段）、法家三小部（204 段）、韓非子（781 段）、管子（668 段）、名家黃老小部四部（182 段）、儒家著述組五部（506 段）；風俗通義（230 段）立下 §2.7 考辨體閘門；**其後連續四部不立新閘門而用既有七道組合判完**——獨斷（152 段，§2.3＋§2.6②＋§2.7）、蔡中郎集（207 段，碑誄體）、太玄經（880 段）與京氏易傳（784 段）（象數／占筮體，兩部都「一書兩道閘門」）。**三巨頭（說文 6,070／戰國策 1,034／水經注 933）已全部標完，巨型書階段結束。****其後兩部（呂氏春秋 736 段、墨子 606 段）確立「一部書一道閘門是巧合不是規則」**——兩部都在 SPEC 裡列「章名 → 體例群 → 閘門」分派表把章分派給五個體例群，墨子更用到四道不同閘門；墨子的墨語群 88／88 零判空是本庫唯一。**分派表第三次用在晏子春秋（218 段），首次出現「兩群命中率幾乎相同」（99%／98%）——分派表擋掉的是問錯問句，不是命中率差異，分完發現一樣也是結果。****文心雕龍（424 段 52 章）是第一部文論，為它現寫 §2.8「怎麼寫之外」閘門（本庫第八道），分派表推到七群，且 G3 創作論在 SPEC 裡就先拆成技術側（12%）／人側（53%）兩個對拍單位——群內再切一層從回收後的診斷提前到寫 SPEC 時做；13／13 全覆蓋、A 類 19 條七批 0 FAIL、509 段引句對拍 0 誤差（本庫首見）。****越絕書（307 段 19 章，2026-09-23）是分派表第五次，六群四道閘門（§2.2／§2.3／§2.5①／§2.6／§2.7②），17 批 0 FAIL、567 段引句 0 誤差。它留下三件事**：①全書判空 62% 不可讀——G1 地志兩章佔 55% 段數且只命中 4%，扣掉後其餘 139 段命中 78%，而**群內 G4 的寶劍 3／3 vs 軍氣 1／17 又差 20 倍**（「報總判空率之前先逐章拆開」在群內部成立）；②**術數詞面最密而 XII 外溢為零**（望氣／占夢／相劍／星野／災異滿書，SPEC 事前五側寫死，XII 實得 2 段、破除側 XI 14 段是它的 7 倍且零互相污染）；③**判讀者中途換人逐批記在 `tagged_by`**（b01–b04 codex-gpt-6-astra、b05–b17 claude-sonnet-4-6，因 codex 撞 7D 牆非品質問題）。**金樓子（515 段 17 章，2026-09-25）是分派表第六次，七群五道閘門，27 批 0 FAIL / 0 NOTE、1311 段引句 0 誤差、13／13 全覆蓋、命中 46%。它留下三件事**：①**引號密集底本要在發包前替每段算出「不含引號的安全片段清單」**——底本 405 處 `「 」`，b18／b20 的 A14 與 FOREIGN 全出自 judge 刪掉原文引號再引，把失敗案例逐字寫進 reason 規則＋對飽和批次事前列安全片段後，連五批零誤差（spec 補一格精準改一格的第三個實例）；②**群內提前切分在 SPEC 階段就做**——G5 登錄側 11／56（20%）vs 自述側 12／23（52%），〈著書篇十〉[1]–[37] 0／37 vs [38]–[52] 11／15（73%），切點是四庫館臣自己說出來的；③**換 judge 要分開評判讀層與引句層**——`minimax-m3` 八批的位置錨點全過，缺陷 100% 落在機械引句要求上。另記一筆 SPEC 內部矛盾（§G5 散文說部類小計要填 `formalization`、line 1095／1184 說書目卷數不是程序），`--check-spec` 全綠因它不讀立場，是這一類漂移的第三個實例。

未標 **25 部、5,930 段**（含 2026-08-24 新收五部）——動工前的分組與優先序見 `HANDOFF.md` §未標存量。

**查一個領域在全庫的段落，看 `00-overview/domains/<id>.md`，不要自己 grep `annotations.json`。**每頁按書分節，欄位是篇名／段序／姿態／摘句／判讀，另附「已通讀但本領域零命中」的書單——命中與零命中放在同一頁，是為了讓零不會被讀成沒人看過（SCHEMA §5）。`DOMAINS.md` 是總表加缺口報告（未標註的 27 部）。三份都由 `build-index.py` 一次生成，不會各自漂移；`verify.py` 每跑一次就對拍一次，過期即 error，所以這頁的數字與 `annotations.json` 不會脫節。
