# 索引

> 本檔由 `scripts/build-index.py` 生成，**不要手改**。
> 生成時間 2026-08-08T21:00:48+00:00｜68 部｜10,553,119 bytes｜已通讀 7 部｜已段落標註 7 部（1943 段）｜僅有骨架待標 0 部（0 段）

## 分類統計

| 類 | 部數 |
|---|---|
| 儒家著述 | 11 |
| 雜史 | 11 |
| 兵家 | 6 |
| 法家 | 5 |
| 術數 | 4 |
| 雜家 | 4 |
| 算書 | 4 |
| 人論 | 4 |
| 雜著 | 4 |
| 志怪 | 3 |
| 醫家 | 3 |
| 名家 | 2 |
| 黃老 | 2 |
| 類書 | 2 |
| 近代 | 1 |
| 地理 | 1 |
| 文論 | 1 |

## 全部文本

| slug | 書名 | 類 | text_role | 章數 | bytes | 通讀 | 段落標註 |
|---|---|---|---|---|---|---|---|
| `jinlouzi` | 金樓子 | 人論 | original | 18 | 197,511 | — | — |
| `renwuzhi` | 人物志 | 人論 | original | 13 | 46,355 | ✓ | 229/229 |
| `shishuo-xinyu` | 世說新語 | 人論 | original | 38 | 597,767 | — | — |
| `yanshi-jiaxun` | 顏氏家訓 | 人論 | original | 20 | 123,091 | ✓ | 255/255 |
| `cai-zhonglang-ji` | 蔡中郎集 | 儒家著述 | original | 170 | 255,990 | — | — |
| `duduan` | 獨斷 | 儒家著述 | original | 36 | 36,623 | — | — |
| `fengsu-tongyi` | 風俗通義 | 儒家著述 | original | 85 | 132,190 | — | — |
| `kongcongzi` | 孔叢子 | 儒家著述 | original | 19 | 96,008 | — | — |
| `qianfulun` | 潛夫論 | 儒家著述 | original | 40 | 180,367 | ✓ | 268/268 |
| `shenjian` | 申鑒 | 儒家著述 | original | 9 | 103,087 | — | — |
| `sushu` | 素書 | 儒家著述 | original | 6 | 5,094 | — | — |
| `taixuanjing` | 太玄經 | 儒家著述 | original | 93 | 96,641 | — | — |
| `xinxu` | 新序 | 儒家著述 | original | 10 | 177,508 | — | — |
| `zhongjing` | 忠經 | 儒家著述 | original | 18 | 8,889 | — | — |
| `zhonglun` | 中論 | 儒家著述 | original | 25 | 73,781 | — | — |
| `liutao` | 六韜 | 兵家 | original | 66 | 64,772 | — | — |
| `san-lue` | 三略 | 兵家 | original | 3 | 14,993 | — | — |
| `sima-fa` | 司馬法 | 兵家 | original | 5 | 13,094 | — | — |
| `sunzi-bingfa` | 孫子兵法 | 兵家 | original | 14 | 30,954 | ✓ | 91/91 |
| `weiliaozi` | 尉繚子 | 兵家 | original | 20 | 18,149 | — | — |
| `wuzi` | 吳子 | 兵家 | original | 6 | 18,009 | — | — |
| `gongsun-longzi` | 公孫龍子 | 名家 | original | 7 | 34,659 | — | — |
| `lusheng-mobian-zhuxu` | 魯勝墨辯注敘 | 名家 | original | 1 | 1,091 | — | — |
| `shuijingzhu` | 水經注 | 地理 | original | 137 | 1,131,449 | — | — |
| `dongmingji` | 洞冥記 | 志怪 | original | 4 | 19,558 | — | — |
| `gu-sanfen` | 古三墳 | 志怪 | contested | 11 | 20,377 | — | — |
| `shenyijing` | 神異經 | 志怪 | original | 10 | 16,004 | — | — |
| `wenxin-diaolong` | 文心雕龍 | 文論 | original | 52 | 148,000 | — | — |
| `guanzi` | 管子 | 法家 | original | 81 | 472,126 | — | — |
| `hanfeizi` | 韓非子 | 法家 | original | 59 | 404,007 | — | — |
| `jian-zhu-ke-shu` | 諫逐客書 | 法家 | original | 1 | 2,989 | — | — |
| `shangjunshu` | 商君書 | 法家 | original | 25 | 73,476 | — | — |
| `shenzi` | 慎子 | 法家 | original | 9 | 25,740 | — | — |
| `haidao-suanjing` | 海島算經 | 算書 | original | 1 | 5,469 | ✓ | 24/24 |
| `jiuzhang-suanshu` | 九章算術 | 算書 | original | 10 | 93,135 | ✓ | 720/720 |
| `sunzi-suanjing` | 孫子算經 | 算書 | original | 4 | 35,563 | — | — |
| `zhoubi-suanjing` | 周髀算經 | 算書 | original | 6 | 22,323 | — | — |
| `chuci` | 楚辭 | 術數 | original | 29 | 60,505 | — | — |
| `jiaoshi-yilin` | 焦氏易林 | 術數 | original | 4 | 18,321 | — | — |
| `jingshi-yizhuan` | 京氏易傳 | 術數 | original | 67 | 89,424 | — | — |
| `shanhaijing` | 山海經 | 術數 | original | 41 | 143,349 | — | — |
| `houheixue` | 厚黑學 | 近代 | original | 22 | 36,610 | — | — |
| `jinkui-yaolue` | 金匱要略 | 醫家 | original | 31 | 102,687 | — | — |
| `nanjing` | 難經 | 醫家 | original | 81 | 45,958 | — | — |
| `shanghanlun` | 傷寒論 | 醫家 | original | 24 | 126,274 | — | — |
| `lienuzhuan` | 列女傳 | 雜史 | original | 104 | 146,795 | — | — |
| `mu-tianzi-zhuan` | 穆天子傳 | 雜史 | original | 8 | 49,338 | — | — |
| `wuyue-chunqiu` | 吳越春秋 | 雜史 | original | 39 | 151,717 | — | — |
| `xijing-zaji` | 西京雜記 | 雜史 | original | 132 | 54,738 | — | — |
| `yandanzi` | 燕丹子 | 雜史 | original | 4 | 11,568 | — | — |
| `yantielun` | 鹽鐵論 | 雜史 | original | 60 | 184,615 | ✓ | 356/356 |
| `yanzi-chunqiu` | 晏子春秋 | 雜史 | original | 215 | 168,105 | — | — |
| `yi-zhoushu` | 逸周書 | 雜史 | original | 66 | 170,111 | — | — |
| `yuejueshu` | 越絕書 | 雜史 | original | 19 | 116,056 | — | — |
| `zhanguoce` | 戰國策 | 雜史 | original | 511 | 1,399,836 | — | — |
| `zhushu-jinian` | 竹書紀年 | 雜史 | contested | 6 | 52,376 | — | — |
| `dengxizi` | 鄧析子 | 雜家 | original | 2 | 11,380 | — | — |
| `guiguzi` | 鬼谷子 | 雜家 | original | 21 | 118,764 | — | — |
| `lushi-chunqiu` | 呂氏春秋 | 雜家 | original | 160 | 398,806 | — | — |
| `yinwenzi` | 尹文子 | 雜家 | original | 4 | 24,718 | — | — |
| `qijing` | 棋經 | 雜著 | original | 15 | 8,300 | — | — |
| `sanzijing` | 三字經 | 雜著 | original | 1 | 4,256 | — | — |
| `songjingwen-gong-biji` | 宋景文公筆記 | 雜著 | original | 7 | 40,023 | — | — |
| `yulizi` | 郁離子 | 雜著 | original | 182 | 155,705 | — | — |
| `qunshu-zhiyao` | 群書治要 | 類書 | original | 347 | 1,608,971 | — | — |
| `yilin` | 意林 | 類書 | original | 104 | 180,522 | — | — |
| `heguanzi` | 鶡冠子 | 黃老 | original | 19 | 57,301 | — | — |
| `yuzi` | 鬻子 | 黃老 | original | 14 | 19,151 | — | — |
